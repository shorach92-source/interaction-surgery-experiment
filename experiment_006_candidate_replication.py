from __future__ import annotations

import argparse
import json
from pathlib import Path

from experiment_006 import LiveBackend, run


def main():
    p = argparse.ArgumentParser()
    p.add_argument("--model", default="gpt-5.4-mini")
    p.add_argument("--scenario", default="D3-02")
    p.add_argument("--repetitions", type=int, default=5)
    p.add_argument("--bootstrap-samples", type=int, default=5000)
    p.add_argument("--seed", type=int, default=6006)
    p.add_argument("--output", default="results_006_candidate")
    a = p.parse_args()

    scenarios = json.loads(Path("scenarios_v03.json").read_text(encoding="utf-8"))
    selected = [s for s in scenarios if s["id"] == a.scenario]
    if len(selected) != 1:
        raise SystemExit(f"Expected exactly one scenario {a.scenario!r}; found {len(selected)}")

    raw, summary = run(
        selected,
        LiveBackend(a.model),
        a.repetitions,
        a.bootstrap_samples,
        a.seed,
    )

    out = Path(a.output)
    out.mkdir(parents=True, exist_ok=True)
    (out / "rollouts.jsonl").write_text(
        "\n".join(json.dumps(x, ensure_ascii=False) for x in raw) + "\n",
        encoding="utf-8",
    )
    (out / "interactions.json").write_text(
        json.dumps(summary, indent=2), encoding="utf-8"
    )

    x = summary[0]
    lines = [
        "# Experiment 006 candidate replication",
        "",
        f"Scenario: `{a.scenario}`; model: `{a.model}`; repetitions/subset: {a.repetitions}; rollouts: {len(raw)}",
        "",
        f"- I utility: {x['utility_interaction']:.3f} [{x['utility_ci95'][0]:.3f}, {x['utility_ci95'][1]:.3f}]",
        f"- I harm: {x['harm_interaction']:.3f} [{x['harm_ci95'][0]:.3f}, {x['harm_ci95'][1]:.3f}]",
        f"- Mixed-sign candidate: {x['mixed_sign_candidate']}",
    ]
    (out / "report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"completed {len(raw)} rollouts for {a.scenario}; results in {out}")


if __name__ == "__main__":
    main()
